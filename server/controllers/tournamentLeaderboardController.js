  ...
  try {
    ...
    const [tournaments, total] = await Promise.all([
      Tournament.find({ isPrivate: { $ne: true } })
        .select("title tournamentLogo type sportsType eventLocation startDate endDate tournamentStatus roundTwoMode currentStage createdAt")
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
    ...
    const tournamentsWithMetadata = tournaments.map((tournament) => {
      ...
      const isGroupStage = tournament.type?.toLowerCase().includes("group stage");
