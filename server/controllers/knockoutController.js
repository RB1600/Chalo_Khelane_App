...
exports.generateNextRound = async (req, res) => {
  try {
    ...
    // Multi-sport: derive sportId from the prior round's first match (so the
    // new round inherits the same sport scoping). Falls back to body and
    // tournament.sports[0] via the factory's _stamp.
    const _priorSportId = completedMatches.find((m) => m.sportId)?.sportId
      || req.body.sportId
      || null;
