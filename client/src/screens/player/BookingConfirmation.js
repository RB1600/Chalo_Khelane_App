    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollContainer}>
        ...
        >
          ...
          {/* Selected Categories / Sports & Categories Section. STEP 12c —
              when sportSelections is populated (multi-sport bookings post
              STEP 12), render a per-sport grouped breakdown with the
              backend-authoritative totalFee. Falls back to the legacy flat
              category list for older bookings without sportSelections. */}
          {(() => {
            const ss = bookingDetails?.sportSelections || [];
            const sc = bookingDetails?.selectedCategories || [];
            if (ss.length === 0 && sc.length === 0) return null;
            ...
            // Legacy fallback — flat category list, unchanged from pre-STEP 12c.
            return (
              <>
                <View style={styles.receiptDivider} />
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Selected Categories</Text>
                  {sc.map((category, index) => {
