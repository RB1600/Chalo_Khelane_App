...
const TournamentDetails = ({ route, navigation }) => {
  ...
  // STEP 12a — Multi-sport: build per-sport sections. Reads from
  // tournament.sports[] when present (post-migration); falls back to a
  // synthesized single-sport section from the root scalar
  // tournament.category (singular) for legacy un-migrated tournaments.
  // Each category in a section is tagged with sportId/sportName so the
  // selection state knows which sport each pick belongs to.
  const sportSections = useMemo(() => {
    ...
    // Legacy fallback — synthesize one section from root scalars.
    ...
    const legacySportName =
      tournament.rawData?.sportsType || tournament.sportsType || item?.sportsType || "Tournament";
    return [{
      sportId: "", // unresolved on legacy tournaments — STEP 9d migration backfills
      sportName: legacySportName,
      sportSlug: "",
      categories: cats
        .filter((c) => c && (c.name || c.categoryName || c.title))
        .map((c) => ({
  ...
  }, [tournament.rawData, tournament.sports, tournament.category, item]);

  // Flat list of categories across all sports — preserves the existing
  // selection-state contract (still uses unique `_id` keys). Each entry
  // carries sportId/sportName so the picker knows which section it belongs to.
            ...
            <ScrollView style={styles.sheetList} showsVerticalScrollIndicator={false}>
              {sportSections.map((section, sIdx) => (
                <View key={section.sportId || section.sportName || sIdx}>
                  {/* Sport section header — only shown when there's more than
                      one section (avoids visual noise for legacy/single-sport). */}
                  {sportSections.length > 1 && (
                    <View style={{ paddingVertical: 8, paddingHorizontal: 4, marginTop: sIdx === 0 ? 0 : 12 }}>
                      <Text style={{ fontSize: 13, fontWeight: '700', color: '#455A64', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                        {section.sportName}
                      </Text>
                        ...
                        <View style={styles.sheetItemMain}>
                          ...
                          <View style={{ marginLeft: 15, flex: 1 }}>
                            ...
                            <Text style={styles.sheetItemFee}>
                              {Number(cat.fee || 0) === 0 ? 'Free entry' : `Registration: ₹${cat.fee}`}
                            </Text>
                          </View>
                        </View>
                      </TouchableOpacity>
            ...
            <View style={styles.sheetFooter}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  // STEP 12a — Multi-sport: build the new sportSelections shape
                  // alongside the legacy selectedCategory shape. BookingScreen
                  // uses sportSelections for the API submit; selectedCategory
                  // remains for the existing review/display logic.
